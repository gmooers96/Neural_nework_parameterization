...
def reshaper_v2(array):
    if len(array.shape) == 2:
        return np.swapaxes(np.expand_dims(array.values.flatten(), axis=0),0,1)
    if len(array.shape) == 3:
        return np.swapaxes(array.values.reshape((len(array), len(array[0])*len(array[0][0]))),0,1)
...
def LoadDataStandardScaleData_v2(traindata,
    ...
    train_variables = xr.open_dataset(traindata, chunks=chunk)
