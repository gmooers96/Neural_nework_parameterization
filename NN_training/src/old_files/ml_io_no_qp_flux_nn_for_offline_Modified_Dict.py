...
def build_training_dataset(expt, start_time, end_time, interval, n_x_samp=5, train_size=0.9, do_shuffle=True,
    ...
    if is_cheyenne == False:  # On aimsir/esker
        #Commmented out by Griffin for work on Bridges2
    ...
    # Griffin Comment -- need to adapt this to Bridges2
    ...
    # what is this doing? this is finding files
    # need to extract file names
    # is this the input path basically? I'm going to comment it out and treat is that way
    ...
    filename_wildcard = input_dir + '*.nc4'
    ...
    if expt == 'qobs':
        ...
        sys.exit()
    # Commented out SST stuff -- Griffin
    #int_count = 0
    #ssty_coarse = []
    ...
    # Initialize
    # Griffin comment -- loop over all the file times
    # could chsnge this
    ...
    #    terra = np.zeros((n_z_input, n_y, n_x_samp, n_files))
    ...
    for ifile, file_time in enumerate(file_times):
        ...
        # Get filename
        # here we use the file name var to find every coarsed data file
        #Commented out by Griffin
        ...
        # Griffin comment -- change var names here to match what I want
        f = Dataset(filename[0], mode='r')
        ...
        # Griffin Comment - I think Q and QV are the Same so I am modifying the line below
        else:
            #qt = (f.variables['Q'][:] + f.variables['QN'][:]) / 1000.0  # total non-precip water (kg/kg)
            qt = (f.variables['QV'][:] + f.variables['QC'][:] + f.variables['QI'][:]) / 1000.0  # total non-precip water (kg/kg)
        if flag_dict['qn_resolved_as_var']:
            qn = f.variables['QN'][:] / 1000.0
        # Griffin comment -- modified based on how qp is calculkated in gsam main loop.py
        #qp = f.variables['QP'][:] / 1000.0  # precipitating water (kg/kg)
        qp = (f.variables['QR'][:] + f.variables['QG'][:] + f.variables['QS'][:]) / 1000.0  # precipitating water (kg/kg)
        
        # Griffin comment -- I don't have these variables, don't think I need them so commenting out
        ...
        # griffin comment -- double check units here
        ...
        w = f.variables['W'][:]  # m/s
        precip = f.variables['PREC'][:]  # precipitation flux kg/m^2/s
        ...
        f.close()
        # griffin comment -- could remove flag or delete code  -- do I want horz. wind - prob not now
        if flag_dict['do_hor_wind_input']:
        ...
        if flag_dict['do_dqp']:
            zt_auto_out[:, :, :] = dqp_t[:, :, :]
            zq_auto_out[:, :, :] = - dqp[:, :, :]
        # Griffin comment -- (based on what I want to predict) need to check and calculate 
        # e.g. flux from tendency
        # convservation enforced by predicting fluxes
        # tend. calculated when model is run online
        if flag_dict['do_q_surf_fluxes_out']:
        ...
        # Griffin Comment --- check the paper (Eq. S5 -- SI of Nature Comms) to understand this
        # when precip. water is dropped, where do we have to modify as a result 
        # we probably want ot model this -- but check the significance of the term -- if everything fails this is a good place to go and change
        # Griffin -- I commented out for now
        ...
        for j in range(zTin.shape[1]):
            ...
            #Griffin comment -- added in a squeeze because zqin is odd shape -- I think has rto do with zqin = qt in the 400s
    ...
    if flag_dict['Tin_feature']:
        print('Tin_feature')
        train_input_list['Tin_feature'] = np.float32(Tin[:, :, randind_trn])
        test_input_list['Tin_feature'] = np.float32(Tin[:, :, randind_tst])
    if flag_dict['qin_feature']:
        print('qin_feature')
        train_input_list['qin_feature'] = np.float32(qin[:, :, randind_trn])
        test_input_list['qin_feature'] = np.float32(qin[:, :, randind_tst])
    if flag_dict['qn_resolved_as_var']:
        print('qn_resolved_as_var')
        train_input_list['qn_resolved_as_var'] = np.float32(qnin[:, :, randind_trn])
        test_input_list['qn_resolved_as_var'] = np.float32(qnin[:, :, randind_tst])
    if flag_dict['Tin_z_grad_feature']:
        print('Tin_z_grad_feature')
        T_grad_in_tr = create_z_grad_plus_surf_var(Tin[:, :, randind_trn])
        train_input_list['Tin_z_grad_feature'] = T_grad_in_tr
        T_grad_in_test = create_z_grad_plus_surf_var(Tin[:, :, randind_tst])
        test_input_list['Tin_z_grad_feature'] = T_grad_in_test
    if flag_dict['qin_z_grad_feature']:
        print('qin_z_grad_feature')
        q_grad_in_tr = create_z_grad_plus_surf_var(qin[:, :, randind_trn])
        train_input_lis['qin_z_grad_feature'] = q_grad_in_tr
        q_grad_in_test = create_z_grad_plus_surf_var(qin[:, :, randind_tst])
        test_input_list['qin_z_grad_feature'] = q_grad_in_test
    if flag_dict['Tin_s_diff_feature']:
        print('Tin_s_diff_feature')
        T_s_diff_in_tr = create_difference_from_surface(Tin[:, :, randind_trn])
        train_input_list['Tin_s_diff_feature'] = T_s_diff_in_tr
        T_s_diff_in_test = create_difference_from_surface(Tin[:, :, randind_tst])
        test_input_list['Tin_s_diff_feature'] = T_s_diff_in_test
    if flag_dict['qin_s_diff_feature']:
        print('qin_s_diff_feature')
        q_s_diff_in_tr = create_difference_from_surface(qin[:, :, randind_trn])
        train_input_list['qin_s_diff_feature'] = q_s_diff_in_tr
        q_s_diff_in_test = create_difference_from_surface(qin[:, :, randind_tst])
        test_input_list['qin_s_diff_feature'] = q_s_diff_in_test
    if flag_dict['do_hor_wind_input']:
        vin[:, 0:np.int(vin.shape[1] / 2), :] = -vin[:, 0:np.int(vin.shape[1] / 2), :]
        if flag_dict['do_momentum_output']:
            print('do_momentum_output')
            uin = vertical_smooth(uin)
            vin = vertical_smooth(vin)
            train_input_list['uin'] = np.float32(uin[:, :, randind_trn])
            train_input_list['vin'] = np.float32(vin[:, :, randind_trn])
            test_input_list['uin'] = np.float32(uin[:, :, randind_tst])
            test_input_list['vin'] = np.float32(vin[:, :, randind_tst])
    if flag_dict['do_ver_wind_input']:
        if flag_dict['do_momentum_output']:
            print('do_momentum_output')
            win = vertical_smooth(win)
            train_input_list['do_momentum_output'] = np.float32(win[:, :, randind_trn])
            test_input_list['do_momentum_output'] = np.float32(win[:, :, randind_tst])
        else:
            train_input_list['do_momentum_output'] = np.float32(win[:, :, randind_trn])
            test_input_list['do_momentum_output'] = np.float32(win[:, :, randind_tst])
    if flag_dict['do_surf_wind']:
        print('do_surf_wind')
        train_input_list['do_surf_wind'] = np.float32(uAbsSurfin[:, randind_trn])
        test_input_list['do_surf_wind'] = np.float32(uAbsSurfin[:, randind_tst])
    if flag_dict['dist_From_eq_in']:
        print('do_surf_wind')
        train_input_list['dist_From_eq_in'] = np.float32(albedo_rad[:, randind_trn])
        test_input_list['dist_From_eq_in'] = np.float32(albedo_rad[:, randind_tst])
    if flag_dict['solin_in']:
        print('solin_in')
        train_input_list['solin_in'] = np.float32(solin_input[:, randind_trn])
        test_input_list['solin_in'] = np.float32(solin_input[:, randind_tst])
    if flag_dict['sst_in']:
        print('sst_in')
        train_input_list['sst_in'] = np.float32(sst_input[:, randind_trn])
        test_input_list['sst_in'] = np.float32(sst_input[:, randind_tst])

    if flag_dict['predict_tendencies']:
        print('predict_tendencies')
        train_input_list['Tout'] = np.float32(Tout[0:flag_dict['input_upper_lev'],:, randind_trn])
        test_input_list['Tout'] = np.float32(Tout[0:flag_dict['input_upper_lev'],:, randind_tst]) 
        
        train_input_list['T_adv_out'] = np.float32(T_adv_out[1:flag_dict['input_upper_lev'], :, randind_trn])
        test_input_list['T_adv_out'] = np.float32(T_adv_out[1:flag_dict['input_upper_lev'], :, randind_tst])
        
        do_sedimentation1 = 1
        do_autoconversion1 = 1
        if do_autoconversion1 :
            print('do_autoconversion1')
            train_input_list['do_autoconversion1'] = np.float32(q_auto_out[0:flag_dict['input_upper_lev'], :, randind_trn])
            test_input_list['do_autoconversion1'] = np.float32(q_auto_out[0:flag_dict['input_upper_lev'], :, randind_tst])
        if do_sedimentation1 :
            print('do_sedimentation1')
            train_input_list['do_autoconversion1'] = np.float32(q_sed_flux_tot[0:flag_dict['input_upper_lev'], :, randind_trn])
            test_input_list['do_autoconversion1'] = np.float32(q_sed_flux_tot[0:flag_dict['input_upper_lev'], :, randind_tst])



    if flag_dict['do_radiation_output']:
        pirnt('do_radiation_output')
        train_input_list['do_radiation_output'] = np.float32(Qradout[0:flag_dict['rad_level'], :, randind_trn])
        test_input_list['do_radiation_output'] = np.float32(Qradout[0:flag_dict['rad_level'], :, randind_tst])
    ...
    if flag_dict['do_z_diffusion_correction']:
        if (flag_dict['do_q_surf_fluxes_out']): 
            ...
            ind_start_diff = 0
        print('do_z_diffusion_correction')
        train_input_list['T_diff_f_out'] = np.float32(T_diff_f_out[ind_start_diff:flag_dict['input_upper_lev'], :, randind_trn])
        test_input_list['T_diff_f_out'] = np.float32(T_diff_f_out[ind_start_diff:flag_dict['input_upper_lev'], :, randind_tst])
        train_input_list['q_diff_f_out'] = np.float32(q_diff_f_out[ind_start_diff:flag_dict['input_upper_lev'], :, randind_trn])
        test_input_list['q_diff_f_out'] = np.float32(q_diff_f_out[ind_start_diff:flag_dict['input_upper_lev'], :, randind_tst])
    if flag_dict['do_q_surf_fluxes_out']:
        print('do_q_surf_fluxes_out')
        train_input_list['do_t_surf_fluxes_out'] = np.float32(tSurfout[:, randind_trn])
        test_input_list['do_t_surf_fluxes_out'] = np.float32(tSurfout[:, randind_tst])
        train_input_list['do_q_surf_fluxes_out'] = np.float32(qSurfout[:, randind_trn])
        test_input_list['do_q_surf_fluxes_out'] = np.float32(qSurfout[:, randind_tst])
    if flag_dict['output_precip']:
        print('output_precip')
        train_input_list['output_precip'] = np.float32(precip_out[:, randind_trn])
        test_input_list['output_precip'] = np.float32(precip_out[:, randind_tst])
    if flag_dict['calc_tkz_z']:
        print('calc_tkz_z')
        train_input_list['calc_tkz_z'] = np.float32(tkh_zout[0:flag_dict['tkz_levels'], :, randind_trn])
        test_input_list['calc_tkz_z'] = np.float32(tkh_zout[0:flag_dict['tkz_levels'], :, randind_tst])
    if flag_dict['do_momentum_output']:
        print('calc_tkz_z')
        train_input_list['u_tend_out'] = np.float32(u_tend_out[:, :, randind_trn])
        train_input_list['v_tend_out'] = np.float32(v_tend_out[:, :, randind_trn])
        train_input_list['w_tend_out'] = np.float32(w_tend_out[:, :, randind_trn])
        test_input_list['u_tend_out'] = np.float32(u_tend_out[:, :, randind_tst])
        test_input_list['v_tend_out'] = np.float32(v_tend_out[:, :, randind_tst])
        test_input_list['w_tend_out'] = np.float32(w_tend_out[:, :, randind_tst])

    train_input_list['y'] = y
    train_input_list['z'] = z
    train_input_list['p'] = p
    train_input_list['rho'] = rho
    
    test_input_list['y'] = y
    test_input_list['z'] = z
    test_input_list['p'] = p
    test_input_list['rho'] = rho
    #Griffin -- Commenting this out for now
    #if rewight_outputs:
    #    print(norm_list)
    #    train_input_list.append(norm_list)
    #    test_input_list.append(norm_list)
    print("Made it here")
    pickle.dump(train_input_list,open(output_dir + expt + data_specific_description + '_NO_DIST_TO_EQ_training.pkl', 'wb'))
    pickle.dump(test_input_list,open(output_dir + expt + data_specific_description + '_NO_DIST_TO_EQ_testing.pkl', 'wb'))
