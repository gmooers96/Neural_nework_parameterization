...
def build_training_dataset(expt, start_time, end_time, interval, n_x_samp=5, train_size=0.9, do_shuffle=True,
    ...
    np.random.seed(123)
    filename_wildcard = input_dir + 'DYAMOND2_coars_9216x4608x74_10s_4608_' + str(flag_dict['starting_phase']) + '_' + str(flag_dict['file_zero_time']) + '.atm.3D_resolved.nc4'
    ...
    for ifile, file_time in enumerate(file_times):
        ...
        #breakpoint()
        filename_wildcard = input_dir + 'DYAMOND2_coars_9216x4608x74_10s_4608_*' + '*_' + zeros + str(file_time) + '.atm.3D_resolved.nc4'
        filename = glob.glob(filename_wildcard)
        print(filename[0])
        ...
        # Griffin commented this out as a test
        ...
        # Griffin Comment -- I think I need to swap out for sigma levels here
        ...
        #Griffin  I think I can get away with commenting this out for now....
        #qpflux_z = f.variables['QP_FLUX_Z'][:] / 1000.0  # SGS qp flux kg/m^2/s
        qpflux_z = f.variables['QP_FLUX_Z_OUT_SUBGRID_SIGMA'][:] / 1000.0  # SGS qp flux kg/m^2/s
        
        # Griffin turned off this flag, can ignore for now
        ...
        # Griffin turned this off, can ignore for now
        ...
        if flag_dict['do_dqp']:
            #Commented out by griffin for now
            #zt_auto_out[:, :, :] = dqp_t[:, :, :]
            zq_auto_out[:, :, :] = - dqp[:, :, :]
        ...
        for j in range(zTin.shape[1]):
            ...
            if do_shuffle:  
                ...
                if len(indexes_x) < n_x_samp:
                    ...
                    #breakpoint()
                    #Griffin Commented out, I don't want a random sample of latitude points on lon band
    ...
    #Just to be safe keeping the old version
    #if flag_dict['cos_lat']:
        #cos_lat = np.moveaxis(cos_lat, 1, 2)
        #cos_lat = np.reshape(cos_lat, (n_y, -1))
    if flag_dict['cos_lat']:
        cos_lat = np.expand_dims(cos_lat, axis=0)
        cos_lat = np.moveaxis(cos_lat, 2, 3)
        cos_lat = np.reshape(cos_lat, (1, n_y, -1))
    #if flag_dict['sin_lon']:
    #    sin_lon = np.moveaxis(sin_lon, 1, 2)
    #    sin_lon = np.reshape(sin_lon, (n_x, -1))
    if flag_dict['sin_lon']:
        #breakpoint()
