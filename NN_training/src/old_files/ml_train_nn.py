...
def train_wrapper(f_ppi, o_ppi, training_expt, input_vert_dim, output_vert_dim,
    ...
    if not only_plot:
        ...
        f, o, y, z, rho, p, weight_list = ml_load.LoadData(trainfile, max_z, input_vert_vars=input_vert_vars, output_vert_vars=output_vert_vars,
                                              rain_only=rain_only, n_trn_exs=n_trn_exs, no_cos=no_cos, use_rh=use_rh,wind_input = do_wind_input,                                             exclusion_flag=flag_dict['exclusion_flag'],ind1_exc=flag_dict['ind1_exc'],ind2_exc=flag_dict['ind2_exc'],rewight_outputs=rewight_outputs)
    ...
    figpath = '/ocean/projects/ees220005p/gmooers/Figures/NN_offline_figs/' + est_str + '/'
...
def PreprocessData_tr_ts(f_ppi, f, test_f, o_ppi, o, test_o, pp_str, n_trn_exs, z, input_vert_dim,
    ...
    # takes in the array, its keys, and vertical sizes and remakes a dictionary
    f_dict = ml_load.unpack_list(f, input_vert_vars, input_vert_dim)
    # creates a dict of scalars matching all the inputs; fits scaling to each data, returns dict of scalars
    ...
    # puts the data together again
    f = ml_load.pack_list(f_dict,input_vert_vars)

    #Preprocessing test features -- same as above except now one step can be skipped because test
...
def train_nn(net, est_str, f_scl, o_scl, tf_scl,to_scl, output_vert_dim, output_vert_vars,epochs =1, min_lr = 2e-4, max_lr = 2e-3, step_size_up=4000,batch_size = 1024):
    # o_scl shape is (932088, 245)
    y_train_small_py = torch.from_numpy(o_scl.reshape(-1, o_scl.shape[1])).float()
    # y_train_small_py shape is torch.Size([932088, 245])
    # f_scl shape is (932088, 151)
    X_norm_py = torch.from_numpy(f_scl.reshape(-1, f_scl.shape[1])).float()
    # X_norm_py shape is ([932088, 151])
    # tf_scl shape is (49416, 151)
    X_train_val_norm_py = torch.from_numpy(tf_scl.reshape(-1, tf_scl.shape[1])).float()
    # X_train_val_norm_py is ([49416, 151])
