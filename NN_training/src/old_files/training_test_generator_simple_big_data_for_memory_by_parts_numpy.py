...
def build_training_dataset(filepath,
    ...
    if flag_dict['predict_tendencies']:
        ...
        my_dict_train["q_sed_flux_tot"] = (("z","lat","sample"), q_sed_flux_tot.compute().to_numpy())
    
    if flag_dict['land_frac']:
        terra = da.from_array(terra, chunks=new_chunks) 
        terra = terra.reshape(n_z_input, n_y, n_files * n_x)
        my_dict_train["terra"] = (("z","lat","sample"), terra.compute().to_numpy())
        del terra
    ...
    if flag_dict['sfc_pres']:
        ...
        my_dict_train["sfc_pres"] = (("lat","sample"), sfc_pres.compute().to_numpy())
        del sfc_pres
    
    if flag_dict['skt']:
        skt = da.from_array(SKT, chunks=small_chunks) 
        skt = skt.reshape(n_y, n_files * n_x)
        my_dict_train["skt"] = (("lat","sample"), skt.compute().to_numpy())
        del skt
    ...
    my_weight_dict = {}
    norm_list = calculate_renormalization_factors(Tout,
                                                       T_adv_out,
                                                       q_adv_out,
                                                       q_auto_out,
                                                       q_sed_flux_tot,
