...
def train_wrapper(config_file):
    ...
    train_inputs_tensors_corrected = torch.swapaxes(train_inputs_tensors, 0, 1)
    train_outputs_tensors_corrected = torch.swapaxes(train_outputs_tensors, 0, 1)
    test_inputs_tensors_corrected = torch.swapaxes(test_inputs_tensors, 0, 1)
    test_outputs_tensors_corrected = torch.swapaxes(test_outputs_tensors, 0, 1)
    
    train_dataset = torch.utils.data.TensorDataset(train_inputs_tensors_corrected, train_outputs_tensors_corrected)
    test_dataset = torch.utils.data.TensorDataset(test_inputs_tensors_corrected, test_outputs_tensors_corrected)
